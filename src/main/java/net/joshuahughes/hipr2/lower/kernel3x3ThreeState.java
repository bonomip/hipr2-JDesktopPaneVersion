...
/**
   ...
   */
  String type = new String("Kernel3x3ThreeState");
  /**
   ...
   */
  public kernel3x3ThreeState(JPanel panel, linkData links){
    ++number;
    setName(type+"_"+number);
    setType(type);
    setParameters();
    setBox(panel,links,0,1);
    box.getOut1().setText("K3");
    go();
  }
  ...
  void setParameters(){
    ...
    JLabel label0 = new JLabel("0 is background");
    JLabel label1 = new JLabel("1 is foreground");
    JLabel label2 = new JLabel("2 is don't care");
    panel.add(label0);
    panel.add(label1);
    panel.add(label2);
  ...
  class buttonListener implements ActionListener{
	...
	((JButton) e.getSource()).setText("2");
	((JButton) e.getSource()).setName("2");
      }else if(((JButton) e.getSource()).getText()=="0"){
