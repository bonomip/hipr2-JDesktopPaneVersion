...
public class ImageAnd extends Thread implements Serializable{
...
//the width of the input images in pixels
   ...
   *@param NAND Boolean to indicate wether we should NAND rather than AND
   ...
   *@return A pixel array containing the AND/NAND of the two input images
   */
   ...
   *@param oset The offset value
   *@param scale The scale value
   *@return A pixel array with the constant value (N)ANDed with the input array
   */
  
  public int [] doAnd(int [] src1_1d, int constant, boolean NAND,
		      int width, int height, float oset, float scale ){
    ...
    /* Due to Java not having unsigned ints, the ~ will always give a
       ...
       the int, I will just mask out the bottom bits. Neil.
       */
    //now do the AND on the input image 
