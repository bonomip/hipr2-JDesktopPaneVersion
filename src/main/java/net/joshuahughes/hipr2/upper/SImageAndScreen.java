...
//The operator class for performing image logical AND. It's a thread
  ...
  private ComboListener clnand = new ComboListener();
  ...
  private JLabel nandlabel = new JLabel( "Operator" );
  private JComboBox nandtoggle = new JComboBox();
  ...
  private JButton startbutton = new JButton("AND");
  ...
  public void add_extra() {
    ...
    imageand= new ImageAnd(i1_w, i2_w);
    ...
    midc. gridx = 0;
    midc. gridy = 2;
    midlayout. setConstraints( nandlabel, midc );
    mid. add( nandlabel );
    
    nandtoggle. addItem("AND");
    nandtoggle. addItem("NAND");
    midc. gridx = 1;
    midc. gridy = 2;
    midlayout. setConstraints( nandtoggle, midc);
    mid. add( nandtoggle );
    nandtoggle. addActionListener(clnand);
 ...
 /**
  ...
  class ComboListener implements ActionListener {
	...
	constant = true;
      ...
      else if( cb.getSelectedItem().equals("NAND")){
	startbutton.setText( "NAND" );
      }
      else if( cb.getSelectedItem().equals("AND")){
	startbutton.setText("AND");
      }
    }
  }
 ...
 /**
  ...
  class ButtonListener implements ActionListener {
	...
	nand = nandtoggle.getSelectedItem().equals( "NAND" );
	try{
	  ...
	  //We want to AND a constant value with image 1
	  
	  if (constant) {
	    dest_1d = imageand.doAnd( src1_1d, constantvalue, nand, i1_w, i1_h,
				      offsetvalue, scalevalue );
	    dest = createImage(new MemoryImageSource(i1_w, i1_h,dest_1d,0, i1_w));
	    d_w = i1_w;
	    d_h = i1_h;
	  } else{
	    dest_1d = imageand.doAnd( src1_1d, src2_1d, nand, d_w, d_h,
				      offsetvalue, scalevalue );
  ...
  public String getAppletInfo() {
    return "Image Logical (N)AND Applet"+
